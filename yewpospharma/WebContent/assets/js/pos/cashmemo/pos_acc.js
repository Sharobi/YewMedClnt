...
var on_bill_return_flag=0;//this is very importent
	...
	if (itemdetail.length == 0) {
		...
		for ( var i = 0; i < itemdetail.length; i++) {
			...
			if (itmstkdet.stockQty == 0) {
				...
				var endtrline = "</tr>";
				createdrowline = starttrline + batchno + expiryDateFormat + stockQty + holdQty + mrppack + mrp + conversion + packing + endtrline;
...
//add customer directly from sale page form
function addEditCustomerDirect() {
	document.getElementById('alertMsg').innerHTML = '';
	var pin = "";
	var name = $('#salecustname').val();
	var code = "";
	var qualification = "";
	var speciality = "";
	var addrs = $('#salecustaddr').val();
	var city = "";
	var state = "";
	var country = "";
	var phn = $('#salecustph').val();
	var fax = "";
	var slgender = "";
	var opbal = 0;
	var creditLimit = 0.0;
	
	if (Number($("#salecustph").val().length) < 10) {
	...
	if (Number($("#salecustaddr").val().length) <=0) {
		document.getElementById('alertMsg').innerHTML = "Please enter Customer name.";
		$(this).focus();
		return false;
	} else {
		document.getElementById('alertMsg').innerHTML = "";
...
//add customer from popup with full details
