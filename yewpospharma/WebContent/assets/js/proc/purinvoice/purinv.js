...
function calculateSpclDisc()
{
	...
	$('#temp_supplier_amt_backup').val(0.0);
	$('#suppbillamt').val(0.0);
	$('#othadjamt').val(0);
	$('#cr_note').val(0.0);
...
//		$("#totnetamnt").val(parseFloat(nettotal).toFixed(2));
	...
	/*	
		...
		if (temp_supp_bil_amt<=0) {
			...
			$('#creditor_amt').val( (parseFloat( tem_net_total-$('#cr_note').val())).toFixed(2));	
		}
		else {
			$('#cr_note').val(0.0);
	...
	var temp_advance_amt= $('#cr_note').val();
	...
	$("#edit_btn").click(function() {
		...
		else
		{
			...
			var totmrp = (Number(pqty) + Number($("#free").val())) * $("#mrp").val();
		...
		$('#creditor_amt').val(parseFloat( $('#totnetamnt').val()).toFixed(2));
		console.log("nettotaledit"+ $('#totnetamnt').val());	
	...
	$("#add_btn").click(function() {
		...
		if (itempresent == 1) {
			...
			tr = tr + row1 + row2 + row3 + row4 + row5 + row6 + row7 + previousFree + row8 + row9 + tbl_sale_rate + row22 + row11 + row24 + row13 + row14 + row15 + netamt + row16 + row17 + row18 + row19 + row20 + row21 + row23 + taxid + taxmode + isgrptax + row25 + row26 + row27 + row28 + row29 + packunitid + isFreeRow + ltadjRow + sku + hsn + poId + predprbfrspdp + billpackqty + mode + returnReason + '</tr>';
		...
		$('#temp_net_total_amt_backup').val(parseFloat( $('#totnetamnt').val()).toFixed(2));
		$('#creditor_amt').val(parseFloat( $('#totnetamnt').val()).toFixed(2));
		console.log("nettotal"+ $('#totnetamnt').val());
	...
	$("#save_btn").click(function (){
		...
		if($("#itemid").val()==0 || $("#itemid").val()=="")
		{
			...
			if(retOnBillStat==1)
			{
				...
				console.log("roundednetamnt=" + parseFloat(roundednetamnt).toFixed(2));
				var roundoff2 = Number(parseFloat(roundednetamnt)) - Number($("#retadj").val());
				
				PurchaseReturnObj.roundoff = $("#roundoff_ret").val();
				//PurchaseReturnObj.roundoff = roundoff2;
				
				console.log("roundoff2_return=" + parseFloat(roundoff2));
				$("#roundoff_ret").val(parseFloat(roundoff2).toFixed(2));
				...
				//console.log("llllls"+$("#roundoff_ret").val());
			//	PurchaseReturnObj.roundoff = $("#roundoff_ret").val();
			//PurchaseReturnObj.roundoff =parseFloat(roundoff).toFixed(2);
				...
				var error=0;
				 ...
				 if(document.getElementById("invmode").checked==false)
				 {
				 	if(isAccount==1) {
						...
						if (PurchaseObj.credior_ledger_id==0) {
							error=1;
							document.getElementById('alertMsgforSave').innerHTML = "please give creditor ledger ";
							$('#footerErrorModal').modal('show');
						}
				 	}
				 }
		...
		ajaxCallObject.callAjaxPost(BASE_URL + "/purorder/getpurorderdetbyordernoforpi.htm", CommonRelsetmapperObj, function(response) {
			...
			if (purdetails.length == 0 || $("#billno").val()=="") {
				...
				for ( var i = 0; i < purdetails.length; i++) {
					...
					var batch = "<td><input type='text' id='" + purdetail.itemId + "_batch' onkeyup='checkBatchOfOrderItem(this.value,this.id)'/></td>";
					...
					var endtrline = "</tr>";
					createdrowline = starttrline + chkbox + itmname + batch + exp + packQty + mrp + rate + taxper + discper + endtrline;
	...
	$('#searchordermodtable tbody tr').each(function() {
		...
		if (saleretvendorid == 0 || saleretvendorid == itemdetail.distributorId || itemdetail.distributorId==0) {
			...
			if (uniquechk == 1) {
				...
				var row5 = '<td id="tbl_lqty" class="numeric">' + itemdetail.pendingRecLooseQty + '</td>';
				var row6 = '<td id="tbl_ratio" class="numeric">' + itemdetail.conversion + '</td>';
				var row7 = '<td id="tbl_free" class="numeric">' + parseFloat(freevalue).toFixed(2) + '</td>';
				var previousFree = '<td id="tbl_prevfree" class="hide">0</td>';
				var row8 = '<td id="tbl_mrp" class="numeric">' +  itemdetail.mrp + '</td>';
				var row9 = '<td id="tbl_rate" class="numeric">' +  itemdetail.rate + '</td>';
				var row10 = '<td id="tbl_ed" class="numeric">' + 0.0 + '</td>';
				var row11 = '<td id="tbl_tax" class="numeric">' + tax + '</td>';
				var row12 = '<td id="tbl_vat" class="numeric">' + 0.0 + '</td>';
...
$("#excelUploadBillNo").blur(function(){
	var billNo = $(this).val();
...
/*
 * for purchase return calculation 
 */
...
function addForRet() {
	...
	if (itempresent == 1) {
		...
		var taxmode = '<td id="tbl_taxmode" class="hide">' + $("#purtaxmode_ret").val() +
